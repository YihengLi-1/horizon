  ...
  return (
    <div className="campus-page space-y-6">
      <section className="campus-hero">
        ...
        <p className="campus-subtitle">{data ? `${data.courseTitle} · ${data.termName}` : "选课趋势与成绩分布深度分析"}</p>
      </section>
        ...
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-5">
            ...
            <div className="campus-card p-4 lg:col-span-3">
              <h2 className="text-sm font-bold text-slate-900 mb-3">报名时间线（实际 ENROLLED 人数）</h2>
              {timelineMeta.points.length === 0 ? (
                ...
                <svg viewBox="0 0 640 240" className="w-full">
                  ...
                  />
                  {timelineMeta.points.filter((_, i) => i % Math.ceil(timelineMeta.points.length / 8) === 0).map((p) => {
            ...
            <div className="campus-card p-4 lg:col-span-2">
              <h2 className="text-sm font-bold text-slate-900 mb-3">成绩分布 · 退课/W {data.dropCount} 人</h2>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="campus-card p-4">
              <h2 className="text-sm font-bold text-slate-900 mb-4">学生评分摘要</h2>
              {ratings === null ? (
                ...
                <div className="space-y-4">
                  <div className="flex items-end gap-3">
                    ...
                    <div className="pb-1">
                      ...
                      {ratings.recommendPct !== null && (
                        <p className="text-xs text-emerald-600 font-semibold">{ratings.recommendPct}% 愿意推荐</p>
                      )}
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500 mb-1">难度系数</p>
                      <StarBar value={ratings.avgDifficulty} />
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">课业负担</p>
            ...
            {/* Student reviews */}
            <div className="campus-card p-4 lg:col-span-2">
              <h2 className="text-sm font-bold text-slate-900 mb-3">学生评论{reviews.length > 0 ? `（${reviews.length}条）` : ""}</h2>
              {reviews.length === 0 ? (
                ...
                <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                  {reviews.map((rv, idx) => (
                    <div key={idx} className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5">
                      ...
                      <p className="text-sm text-slate-700 leading-snug">{rv.comment}</p>
                    </div>
                  ))}
                </div>
              )}
