...
export default function AdminRequestsClient() {
  ...
  const reviewedFacultySteps = useMemo(
    () =>
      requests.reduce((count, request) => {
        return count + (request.steps.some((step) => step.stepKey === "faculty_review" && step.status === "APPROVED") ? 1 : 0);
      }, 0),
    [requests]
  );
    ...
    <div className="space-y-6">
      <section className="campus-hero">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="campus-eyebrow">Registrar Finalization</p>
            <h1 className="font-heading text-3xl font-bold text-slate-900">Approval Requests</h1>
            <p className="mt-2 text-sm text-slate-600">
              Finalize faculty-reviewed prerequisite override requests. Policy effect applies only after registrar approval.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="campus-kpi border-slate-200 bg-white">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Owned Requests</p>
              <p className="mt-1 text-2xl font-semibold text-slate-900">{pendingCount}</p>
            </div>
            <div className="campus-kpi border-blue-200 bg-blue-50">
              <p className="text-xs font-semibold uppercase tracking-wide text-blue-700">Faculty Reviewed</p>
              <p className="mt-1 text-2xl font-semibold text-blue-900">{reviewedFacultySteps}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {!error && !loading && requests.length === 0 ? (
        <section className="campus-card p-8 text-center text-sm text-slate-500">
          No prerequisite override requests are awaiting registrar finalization.
        </section>
      ) : null}
